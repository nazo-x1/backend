...
class 违约认定人工审核表(db.Model):
    ...
    认定人 = db.Column(db.String(10))
    认定申请时间 = db.Column(db.Time)
...
class 客户表(db.Model):
    ...
    客户名 = db.Column(db.String(8))
    性别 = db.Column(db.String(2))
    区域 = db.Column(db.String(50))
    行业 = db.Column(db.String(20))
...
class 重生人工审核表(db.Model):
    ...
    重生申请时间 = db.Column(db.Time)
    审核状态 = db.Column(db.String(20))
    负责人 = db.Column(db.String(10))
    def to_dict(self):
        model_dict = dict(self.__dict__)
        del model_dict['_sa_instance_state']
        return model_dict
